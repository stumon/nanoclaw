...
async function larkGet(token, endpoint, params) {
...
async function larkPost(token, endpoint, body) {
...
async function larkPut(token, endpoint, body) {
    const res = await fetch(`${BASE_URL}${endpoint}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
...
async function larkDelete(token, endpoint) {
    const res = await fetch(`${BASE_URL}${endpoint}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    });
...
async function deleteRecordsForPlatform(token, records, platform) {
...
// ─── Platform handlers ──────────────────────────────────────────────────────
async function handleReplace(token, records, platform) {
...
async function handleUpsert(token, records, platform) {
